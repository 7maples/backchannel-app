...
describe TrackShow do
  ...
  let(:track_show) { TrackShow.new(1, "reem-conf")}

  before do
    MessageFetcher.should_receive(:for_tracks).and_return([{'user_id' => user.id.to_s, 'body' => 'oh hai'}])
  end
  ...
  describe '#messages' do
    it 'returns messages with associated users in an openstruct' do
      ...
      message = track_show.messages.first
  ...
  describe '#users' do
    it "returns a hash of users associated with messages with user.id as key" do
      expect(track_show.users).to eq({user.id => [user]})
    end
  end

  describe '#track' do
    it "returns the requested track" do
      Backchannel::Scheduler::Gem::ConferenceFetcher.should_receive(:get_track).with(1)
      track_show.track
    end
  end
  ...
  describe '#conference_slug' do
    it "returns the conference_slug for the requested track" do
      expect(track_show.conference_slug).to eq "reem-conf"
    end
  end
end
